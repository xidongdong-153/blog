...
- `app/(site)/`：公开页面，页面私有组件在同级 `_components/` 按功能分组。
- `lib/content.ts`：MDX 内容读取层，frontmatter 校验在这里。
- `content/blog/`、`content/notes/`：文章和笔记源文件。
- `site.config.ts`：站点标题、导航、社交链接。
...
- 新增页面放进 `app/(site)/`，页面专属组件放 `app/(site)/_components/` 对应分组，不建第二套目录结构。
- 日期在数据层一律存 ISO 字符串，渲染时用 `lib/content.ts` 的 `formatDate`，不直接在组件里 new Date 再格式化。
