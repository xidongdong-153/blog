...
现状：整个项目只有一个 client 组件 `app/(site)/_components/site/theme-toggle.tsx`。MDX 渲染走异步 RSC（`mdx-content.tsx` 的 `compileMDX`），不需要 client。
...
参考：`app/(site)/_components/blog/post-card.tsx`。

## Props

- 简单组件用内联类型：`export function PostCard({ post }: { post: BlogPost })`。
- 页面组件的 params 等结构用 interface（如 `BlogPostPageProps`，见 `app/(site)/blog/[slug]/page.tsx`）。
- props 类型从数据层 import（`BlogPost`、`Note` 来自 `lib/content.ts`），不在组件里重复定义数据形状。
...
- 内容宽度统一 `max-w-3xl`，在 `app/(site)/layout.tsx` 的 `<main>` 上，页面内部不再限宽。
...
- 空列表要给文案兜底（`还没有文章。`，见 `app/(site)/blog/page.tsx`）。

## 动态路由页

参考 `app/(site)/blog/[slug]/page.tsx` 的完整结构：
...
正文渲染统一走 `MdxContent`（rehype-slug + remark-gfm），不在别处再调 `compileMDX`。目录（TOC）的锚点 id 依赖 rehype-slug 生成的 id，`lib/content.ts` 的 `extractHeadings` / `slugifyHeading` 与之保持一致，改任何一边都要同步另一边。

## 占位页

未实现的功能统一用 `app/(site)/_components/placeholder/empty-state.tsx`，页面注释里写实现方案（参考 `app/(site)/search/page.tsx`、`giscus-comments.tsx`）。实现后删掉占位组件和注释，并把 README「功能状态」表的状态改成「已实现」。
