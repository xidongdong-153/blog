...
| [内容约定](./content-guidelines.md)      | 文章与笔记字段、slug、日期及校验边界                |
| [功能状态](./feature-status.md)          | 已实现与未开始功能的唯一清单                        |
...
代码修改后，前三条检查全过才算完成，顺序：类型 → lint → format。纯文档修改运行格式检查、相对链接核对和 `git diff --check`；Trellis Markdown 默认被忽略，按[质量规范](./quality-guidelines.md)显式检查。README 只保留上手入口，详细维护说明在本目录维护。
