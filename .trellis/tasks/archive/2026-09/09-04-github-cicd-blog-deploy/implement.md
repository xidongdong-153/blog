...
以上四项已在文档中写明操作步骤，并已在 GitHub 远程设置页确认配置完成。
...
6. 第一次发布验证。
   - [x] 先在服务器完成前置条件和手工 SSH 命令验证；服务器 `src/site.config.ts` 已与 GitHub `main` 对齐，工作区只保留允许的 `pnpm-workspace.yaml`。
   - [x] 在 GitHub 配置 `Deployment` Environment，设置 Variables、Secrets 和分支保护；观察到运行 `33860351255` 的 `quality` 和 `Deploy production` 全部成功。
   - [x] 确认 `quality` 通过后 `deploy` 自动运行，部署日志未输出 secrets。
   ...
   - [x] 推送修复后的 workflow 后重新观察 Actions；运行 `33860351255` 的 `quality` 和 `Deploy production` 均成功，服务器已更新并通过健康检查。
...
- [x] 任务验收标准全部可观察地满足；第一次真实发布运行 `33860351255` 成功，服务器 `HEAD` 与 `origin/main` 均为 `598e09c`。
